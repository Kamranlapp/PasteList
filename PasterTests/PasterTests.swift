...
final class PasterTests: XCTestCase {
    ...
    @MainActor
    func testPostEventAccessIsRequestedOnlyByExplicitAction() {
    ...
    func testOnboardingIsPresentedOnlyBeforeCompletion() throws {
        let suiteName = "PasterTests.onboarding.\(UUID().uuidString)"
    ...
    func testCompletedOnboardingCanStillBeReopenedWithoutResettingState() throws {
        let suiteName = "PasterTests.onboarding.reopen.\(UUID().uuidString)"
        let defaults = try XCTUnwrap(UserDefaults(suiteName: suiteName))
        defer { defaults.removePersistentDomain(forName: suiteName) }
    ...
    func testAutomaticPasteActivatesPreviousApplicationAndPostsCommandV() async {
        ...
        let controller = PasteAutomationController(
            ...
            requestPostEventAccess: { XCTFail("Permission request must be explicit"); return false },
    ...
    func testApplicationSourcesDoNotUseAccessibilityUIAPIs() throws {
        let testsURL = URL(fileURLWithPath: #filePath)
        let projectRoot = testsURL
            .deletingLastPathComponent()
            .deletingLastPathComponent()
        let sourceRoot = projectRoot.appendingPathComponent("Paster", isDirectory: true)
        let forbiddenSymbols = [
            ["AXUI", "Element"].joined(),
            ["AXIs", "ProcessTrusted"].joined(),
        ]
    ...
    func testCursorPanelSizeIsSavedAndRestored() throws {
        let suiteName = "PasterTests.cursorPanelSize.\(UUID().uuidString)"
        let defaults = try XCTUnwrap(UserDefaults(suiteName: suiteName))
        defer { defaults.removePersistentDomain(forName: suiteName) }
    ...
    func testSavedPanelVisibilityIsSavedAndRestored() throws {
        let suiteName = "PasterTests.savedPanelVisible.\(UUID().uuidString)"
        let defaults = try XCTUnwrap(UserDefaults(suiteName: suiteName))
        defer { defaults.removePersistentDomain(forName: suiteName) }
