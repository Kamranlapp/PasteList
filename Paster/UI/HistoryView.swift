...
struct HistoryView: View {
    ...
    private static let listCoordinateSpace = "PasterHistoryList"
    ...
    private func rows(_ clips: [ClipRecord]) -> some View {
        ForEach(Array(clips.enumerated()), id: \.element.id) { index, clip in
            rowContent(
                for: clip,
                quickPasteKey: QuickPasteShortcut.label(forEntryIndex: index)
            )
            .background {
                if let id = clip.id {
    ...
    private func rowContent(
        ...
        case .fileDrag:
                ...
                !urls.isEmpty
            {
                ...
                restoreButton(for: clip, quickPasteKey: quickPasteKey)
            }
