...
enum SavedClipsPlacement {
    ...
    /// Same width as the history panel's visible surface, exactly half its
    /// height, sharing its top edge.
    static func frame(
        ...
        let originX: CGFloat
        if rightOrigin + size.width <= visibleFrame.maxX {
            originX = rightOrigin
